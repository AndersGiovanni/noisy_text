...
bertProbs = np.load('BERT_preds_proba.npy')
...
for i in range(len(bert)):
    instance = [bert[i], bertProbs[i][0], bertProbs[i][1], svm[i], conv[i], convProbs[i], mlp[i], mlpProbs[i]]
...
for fold in range(numFolds):
    ...
    # classifier = RandomForestClassifier(max_depth=2, random_state=0, n_estimators=200)
